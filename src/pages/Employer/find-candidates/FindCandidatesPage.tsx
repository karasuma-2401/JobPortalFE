...
export default function FindCandidatesPage() {
  ...
  const filteredCandidates = useMemo(() => {
    return MOCK_CANDIDATES.filter((candidate) => {
      ...
      const matchCategory =
        selectedCategory === "All" || candidate.role.includes(selectedCategory);
  ...
  return (
    <div className="w-full max-w-7xl mx-auto animate-in fade-in duration-500 pb-16 flex flex-col lg:flex-row gap-8">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-2 gap-6">
            {filteredCandidates.map((candidate) => (
              <CandidateCard
                ...
                onInvite={(name) =>
                  setInviteModalData({ isOpen: true, candidateName: name })
                }
              />
            ))}
          </div>
      ...
      <InviteModal
        ...
        onClose={() => setInviteModalData({ isOpen: false, candidateName: "" })}
