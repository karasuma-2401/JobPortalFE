        ...
        <div className="w-full bg-white border border-gray-200/60 rounded-xl p-6 flex flex-col sm:flex-row justify-between items-start sm:items-center gap-6 shadow-sm">
          <div className="flex items-center gap-5">
            ...
            <div className="space-y-1">
              <h1 className="text-[24px] font-bold text-gray-900">{employerData.name}</h1>
              <p className="text-[14px] text-gray-500">{employerData.category}</p>
            </div>
          </div>
          <button 
          ...
          <div className="lg:col-span-2 space-y-8 bg-white border border-gray-200/60 rounded-2xl p-6 sm:p-8 shadow-sm">
            ...
            {employerData.benefits && employerData.benefits.length > 0 && (
              <div className="space-y-3">
                ...
                <ul className="space-y-2.5 list-disc pl-5 text-[14px] text-gray-500">
                  {employerData.benefits.map((b, i) => <li key={i}>{b}</li>)}
                </ul>
              </div>
            )}
            ...
            {employerData.vision && (
              <div className="space-y-3">
                ...
                <p className="text-[14px] text-gray-500 leading-relaxed">{employerData.vision}</p>
              </div>
            )}
          </div>
          <div className="lg:col-span-1 space-y-6">
            <div className="bg-white border border-gray-200/60 rounded-2xl p-6 shadow-sm space-y-5">
              ...
              <div className="grid grid-cols-2 gap-y-5 gap-x-4">
                <div className="space-y-1"><Calendar size={20} className="text-primary-500" /><p className="text-[11px] text-gray-400 font-bold uppercase">Founded In</p><p className="text-[14px] font-semibold text-gray-800">{employerData.overview.founded}</p></div>
                <div className="space-y-1"><Building2 size={20} className="text-primary-500" /><p className="text-[11px] text-gray-400 font-bold uppercase">Org Type</p><p className="text-[14px] font-semibold text-gray-800">{employerData.overview.orgType}</p></div>
                <div className="space-y-1"><Users size={20} className="text-primary-500" /><p className="text-[11px] text-gray-400 font-bold uppercase">Team Size</p><p className="text-[14px] font-semibold text-gray-800">{employerData.overview.teamSize}</p></div>
                <div className="space-y-1"><Layers size={20} className="text-primary-500" /><p className="text-[11px] text-gray-400 font-bold uppercase">Industry</p><p className="text-[14px] font-semibold text-gray-800">{employerData.overview.industry}</p></div>
              </div>
            </div>
          </div>
        </div>
