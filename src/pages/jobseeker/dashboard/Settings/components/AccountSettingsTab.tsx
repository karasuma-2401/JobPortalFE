  ...
  return (
    <div className="space-y-12 text-left animate-fade-in pb-10">
      ...
      <section className="space-y-6">
        ...
        <Button variant="primary" onClick={() => handleSave("Contact Info")} disabled={isLoading} className="px-8">
           Save Changes
        </Button>
      </section>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-y-4 gap-x-10">
          {[
            ...
            <label key={idx} className="flex items-center gap-3 cursor-pointer group">
              <input type="checkbox" defaultChecked={idx === 0 || idx === 2 || idx === 4} className="w-5 h-5 rounded border-gray-300 text-primary-500 focus:ring-primary-500" />
      ...
      <section className="pt-10 border-t border-gray-100 space-y-6">
        ...
        <Button variant="primary" onClick={() => handleSave("Job Alerts")} className="px-8">Save Changes</Button>
      </section>

      {/* SECTION 4: PRIVACY */}
      <section className="pt-10 border-t border-gray-100 grid grid-cols-1 md:grid-cols-2 gap-10">
         <div className="space-y-4">
            <h3 className="text-sm font-bold text-gray-900">Profile Privacy</h3>
            <div className="flex items-center gap-3 p-1 bg-gray-50 rounded-lg w-fit">
               <button className="px-4 py-2 bg-primary-500 text-white rounded-md text-xs font-bold shadow-sm">YES</button>
               <span className="text-xs text-gray-400 px-2 font-medium">Your profile is public now</span>
               <button className="px-4 py-2 text-gray-400 text-xs font-bold">NO</button>
            </div>
         </div>
         <div className="space-y-4">
            <h3 className="text-sm font-bold text-gray-900">Resume Privacy</h3>
            <div className="flex items-center gap-3 p-1 bg-gray-50 rounded-lg w-fit">
               <button className="px-4 py-2 text-gray-400 text-xs font-bold">YES</button>
               <button className="px-4 py-2 bg-white text-gray-900 rounded-md text-xs font-bold shadow-sm border border-gray-100">NO</button>
               <span className="text-xs text-gray-400 px-2 font-medium italic">Your resume is private now</span>
            </div>
         </div>
      </section>
      ...
      {/* SECTION 5: CHANGE PASSWORD */}
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-5">
            ...
            <div key={field} className="flex flex-col gap-2">
                <label className="text-sm font-medium text-gray-700 capitalize">{field} Password</label>
        ...
        <Button variant="primary" onClick={() => handleSave("Password")} className="px-8">Change Password</Button>
        </section>
      ...
      <section className="pt-10 border-t border-gray-100 space-y-4">
        ...
        <p className="text-sm text-gray-500 leading-relaxed max-w-2xl">
          If you delete your jobseeker account, you will no longer be able to get information about the matched jobs, following employers, and job alerts, shortlisted jobs and applications received from the services of jobpilot.com.
        </p>
