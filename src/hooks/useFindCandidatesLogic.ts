...
import { useSavedCandidates, useSaveCandidate, useRemoveSavedCandidate } from './useSavedCandidates';
...
export function useFindCandidatesLogic() {
    ...
    const [inviteModalData, setInviteModalData] = useState({ isOpen: false, candidateName: '' });
    ...
    // Các API Queries & Mutations
    const { data: candidates = [], isLoading: isDiscoverLoading } = useDiscoverCandidates(filters);
    ...
    const filteredCandidates = useMemo(() => {
        return candidates.filter((candidate) => {
            ...
            if (selectedExperience === '3-5 Years') return years >= 3 && years <= 5;
    ...
    const toggleSaveCandidate = (id: string, name: string) => {
        ...
        if (savedIds.has(id)) {
            removeSavedCandidate(seekerId, { onSuccess: () => toast.info(`Removed ${name} from saved list.`) });
        } else {
            saveCandidate(seekerId, { onSuccess: () => toast.success(`Saved ${name} successfully!`) });
        }
    };
    ...
    return {
        state: { searchQuery, selectedCategory, selectedExperience, selectedCandidate, inviteModalData, filteredCandidates },
        actions: { setSearchQuery, setSelectedCategory, setSelectedExperience, setSelectedCandidate, setInviteModalData, toggleSaveCandidate, handleHireCandidate, clearAllFilters },
